...
Aim: the graph is aiming to show us that the vaccince indeed reduced by a significant amount of cases for various diseases from history.
...
2. The decent usage of hue, identifying the year of vaccince clearly, as well as the shape(scale) of the bubbles, indicating the cases' size.

Disadvantages: 

1. The overlapping of bubbles are severe prblems preventing me to see the patterns clearly, I may group the problem as bad usage of luminance as the luminance in the picture actually means nothing, but it is a bit misleading to me.
...
The graph has balanced view over different kinds of diseases. It shows clearly, directly and a bit urgly using abline to indicate when the vaccince appeared. I think it is more efficient comparing to the bubble plot.
...
From the graph, we can tell that vaccine makes the mean of cases graphcally significantly smaller.

This graph compares the scale by summarizing the data, using length (CI) instead of area. We may also apply a boxplot ot violin plot. However, I have tried the whole afternoon and night, but just don't know how to make the cases to the y axis. So I end up doing this again. 